...
- **Mitigation:** shipped fused confidence (`SHIPPED_CONFIDENCE="fused6"`, `driftsense/calibration.py`) with the found-threshold re-tuned to the fused units (0.4870, downward-biased so present pairs are never wrongly declined; totals 78.45 vs 78.51 at the F1-optimal point — inside draw noise, and 3 vs 6 lost-real present pairs favours the lower threshold).
...
- **Remaining limitation:** organizer hardware differs; final package still requires a clean 4-core CPU timing pass with p50/p90/max recorded. The coarse sweep's remaining FFT-immune cost is template construction (~47% of coarse time; search-side DFT reuse measured at only ~1.5% net, `driftsense/coarse_fft.py`).
