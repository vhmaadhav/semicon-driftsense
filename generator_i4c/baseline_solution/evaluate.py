...
def evaluate_level(level, n_samples, tolerance_px, base_seed):
    ...
    for i in range(n_samples):
        ...
        # no_match_prob=0.0: this evaluation's PR-curve math assumes every
        # sample has exactly one true match to find (see module docstring).
        # Phase 2's other new challenges (reference_rotation_deg,
        # polygon_scale_prob) stay at their real defaults on purpose --
        # rotation/polygon-scale are difficulty the baseline should be
        # measured against, not switched off for the benchmark.
        params = GenerationParams(no_match_prob=0.0, **overrides)
