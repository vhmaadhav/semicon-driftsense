...
class GenerationParams:
    ...
    # Phase 2 --------------------------------------------------------------
    # Whole-image rotation applied only to the captured Reference (not the
    # Search) -- models stage/tool angular misalignment between the two
    # scans, rather than a property of the device. Per-sample angle drawn
    # uniformly from [-x, +x] degrees; 0 disables.
    reference_rotation_deg: float = 10.0

    # Per-polygon scaling "outliers": with probability polygon_scale_prob,
    # an individual drawn feature (contact, line/fin/gate) is additionally
    # scaled by a factor in [1-polygon_scale_range, 1+polygon_scale_range],
    # layered on top of each pattern module's own continuous per-instance
    # jitter. A property of the device (baked into the fine canvas), so
    # both derived images see it consistently -- unlike the acquisition
    # knobs above.
    polygon_scale_prob: float = 0.10
    polygon_scale_range: float = 0.10

    # Probability that a sample's Reference is deliberately cropped from a
    # *different*, independently-generated canvas than the one the Search
    # image came from -- a genuine "no match" case where the Reference does
    # not appear anywhere in the Search image. When this fires, gt_x/gt_y/
    # gt_box come back None and the sample dict's match_found is False.
...
def generate_fine_canvas(
    ...
    return generator(
        ...
        polygon_scale_prob=params.polygon_scale_prob,
        polygon_scale_range=params.polygon_scale_range,
    )
...
def _rotated_reference_crop(
    ...
    """Crop the Reference at (x0, y0) and, if max_angle_deg > 0, rotate it by
    a random angle drawn from [-max_angle_deg, +max_angle_deg] -- models a
    Reference captured at a slight stage/tool angular misalignment relative
    to the Search scan (an acquisition artifact, not a device property, so
    it's applied here rather than baked into the shared fine canvas).

    Crops an oversized patch first so rotating and cropping back down to
    `size` never introduces missing/replicated corners; the oversized patch
    is itself clamped and edge-padded so this is safe near canvas edges.
    """
    if max_angle_deg <= 0:
        return fine_canvas[y0:y0 + size, x0:x0 + size]

    angle = rng.uniform(-max_angle_deg, max_angle_deg)
    margin = int(np.ceil(size * 0.25))
    big = size + 2 * margin
    cx, cy = x0 + size // 2, y0 + size // 2
    bx0, by0 = cx - big // 2, cy - big // 2
    h, w = fine_canvas.shape

    sx0, sy0 = max(bx0, 0), max(by0, 0)
    sx1, sy1 = min(bx0 + big, w), min(by0 + big, h)
    patch = fine_canvas[sy0:sy1, sx0:sx1]

    pad_left, pad_top = sx0 - bx0, sy0 - by0
    pad_right = big - patch.shape[1] - pad_left
    pad_bottom = big - patch.shape[0] - pad_top
    if pad_left or pad_top or pad_right or pad_bottom:
        patch = cv2.copyMakeBorder(patch, pad_top, pad_bottom, pad_left, pad_right, cv2.BORDER_REPLICATE)

    center = (big / 2.0, big / 2.0)
    m = cv2.getRotationMatrix2D(center, angle, 1.0)
    rotated = cv2.warpAffine(patch, m, (big, big), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REPLICATE)
    return rotated[margin:margin + size, margin:margin + size]
...
def generate_sample(
    ...
    if match_found:
        ref_crop = _rotated_reference_crop(fine_canvas, x0, y0, REFERENCE_SIZE_PX, params.reference_rotation_deg, rng)
    else:
        # Reference comes from a genuinely different, independently-generated
        # canvas -- it does not appear anywhere in this sample's Search image.
        neg_zone_result = generate_fine_canvas_zoned(architecture, rng, params)
        neg_x0, neg_y0 = _pick_crop_origin(neg_zone_result, params, rng)
        ref_crop = _rotated_reference_crop(
            neg_zone_result["canvas"], neg_x0, neg_y0, REFERENCE_SIZE_PX, params.reference_rotation_deg, rng
        )
...
def generate_sample_family(
    ...
    *imaged* differs, which is the point: the same Reference should still be
    ...
    ref_crop = _rotated_reference_crop(fine_canvas, x0, y0, REFERENCE_SIZE_PX, params.reference_rotation_deg, ref_rng)
    reference_img = sem_imaging.image_reference(
        ref_crop,
        pixel_size_nm=PIXEL_SIZE_REF_NM,
        spot_size_nm=params.beam_spot_size_nm,
