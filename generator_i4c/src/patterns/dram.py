...
def _line_mask(
    ...
    `polygon_scale_prob`/`polygon_scale_range` inject a sparse, larger
    "outlier" scale on top of the usual continuous jitter: each line
    independently has `polygon_scale_prob` chance of being additionally
    scaled by a factor in [1-range, 1+range] -- a deliberate, more visible
    per-polygon size anomaly (Phase 2), distinct from the dense/subtle
    per-instance jitter every line already gets.
    """
    ...
    if polygon_scale_prob > 0 and polygon_scale_range > 0:
        is_outlier = rng.random(len(positions)) < polygon_scale_prob
        outlier_factor = 1.0 + rng.uniform(-polygon_scale_range, polygon_scale_range, size=len(positions))
        widths = widths * np.where(is_outlier, outlier_factor, 1.0)
...
def generate_dram_canvas(
    ...
    for i, wl in enumerate(word_positions):
        for j, bl in enumerate(bit_positions):
            if (i + j) % 2 == 0:
                outlier = 1.0
                if polygon_scale_prob > 0 and rng.random() < polygon_scale_prob:
                    outlier = 1.0 + rng.uniform(-polygon_scale_range, polygon_scale_range)
                radius = max(1, int(round(base_radius * (1.0 + rng.normal(0, WIDTH_JITTER_FRACTION)) * outlier)))
