...
def main():
    ...
    def resolve(path):
        return path if os.path.isabs(path) else os.path.join(split_dir, path)

    reference = cv2.imread(resolve(row["reference_path"]), cv2.IMREAD_GRAYSCALE)
    search = cv2.imread(resolve(row["search_path"]), cv2.IMREAD_GRAYSCALE)

    search_annotated = cv2.cvtColor(search, cv2.COLOR_GRAY2BGR)
    match_found = row.get("match_found", "True") != "False"
    if match_found and row.get("gt_box_x"):
        ...
        cv2.putText(
            search_annotated, "NO MATCH", (20, 40),
    ...
    gt_str = f"({row['gt_x']}, {row['gt_y']})" if match_found else "NO MATCH"
    print(f"architecture={row['architecture']}  gt={gt_str}")
