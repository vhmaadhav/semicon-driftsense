...
GRID = np.linspace(-4., 4., 33, dtype=np.float32)


def features(search, template, x, y):
    from .matching import row_offsets
    h,w=template.shape
    y0=int(round(y-h/2)); x0=int(round(x-w/2)); ci=int(round(y))-y0
    if not 1 <= ci < h-1:
        return None
    delta=y0-(y-h/2)
    ty=np.tile((np.arange(h,dtype=np.float32)+delta)[:,None],(1,w))
    tx=np.tile(np.arange(w,dtype=np.float32),(h,1))
    aligned=cv2.remap(template.astype(np.float32),tx,ty,cv2.INTER_LINEAR,borderMode=cv2.BORDER_REPLICATE)
    curves=[]
...
def correction(feature, weights):
    if weights['w1'].shape[1] == 138:
        feature=np.concatenate([feature[..., :200].reshape(*feature.shape[:-1],8,25)[...,4:21].reshape(*feature.shape[:-1],136),feature[...,200:]],axis=-1)
