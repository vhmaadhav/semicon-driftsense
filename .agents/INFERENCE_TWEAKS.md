...
> **CORRECTION (audit 2026-09-04): "66.8% of pair time" is a GPU-venv number and
> does not hold on the graded CPU config.** `.agents/CPU_RUNTIME.md:13-16` measured
> **network 90.6% / pose_candidates 8.2%** there; a re-profile of the shipped 1.02M
> checkpoint on pinned deps read **86.1% / 12.2%**. This does not change the E3
> verdict — a wash is a wash — but it explains *why* it was a wash, and it means
> the coarse sweep is **not** "the remaining clock lever". The network is. See the
> matching correction in `.agents/PHASE2_STATE.md`.
