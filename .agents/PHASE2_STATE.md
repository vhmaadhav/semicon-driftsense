...
> **CORRECTION (audit 2026-09-04): the 66.8% / 21.3% split above is wrong for the
> graded configuration, and the "86% network figure is stale" sentence inverts the
> truth.** That profile was taken on `venv313`, a **CUDA-capable** torch build. On
> the CPU-only graded config the ordering reverses — `.agents/CPU_RUNTIME.md:13-16`
> measured **network 90.6% / pose_candidates 8.2%**, and an independent re-profile
> of the shipped 1.02M checkpoint on pinned deps (`scripts/profile_pair.py`, 6
> pairs, 4 threads) read **network 86.1% / pose_candidates 12.2%**. The original
> "86% network" figure was right all along. Consequence: coarse-sweep elimination
> (SEA/FGSE, E3, FFT reuse) is aimed at ~8-12% of the graded cost, which is why
> every one of those measured as a wash. Do not plan optimization work from the
> 66.8% number.
...
> **RESOLVED (audit 2026-09-04): all three of the above are fixed in the code.**
> Verified against the current tree:
> * `scripts/morning_pipeline.sh:68-87` now carries an explicit `reference_px`
>   guard that ABORTs the run unless the shard reports 1000.
> * `scripts/eval_ext.py:390` now declares
>   `choices=["zncc", "consensus", "majority"]`, and its help text records that
>   the earlier `rank`/`band`/`dog` wording was stale.
> * `scripts/apply_rejector.py:69` now calls `np.ptp(shipped)`, with a comment
>   naming the NumPy 2.0 removal.
>
> This block is kept for history. Do not re-file these as open bugs.
