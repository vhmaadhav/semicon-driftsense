...
What the `gat` column actually measures
---------------------------------------
Not symmetric GAT. `estimate_noise_model` needs 4 * 24 = 96 block samples
before it will fit (a, b); a rendered template is TEMPLATE_SIZE = 100 px
square, which is 6x6 = 36 blocks at the default 16 px block, so the template
side falls back to plain Anscombe for every possible input, not merely for
awkward ones. The arm above is therefore **GAT(search) + Anscombe(template)**,
a hybrid, and is labelled as such rather than as the symmetric transform the
issue proposed. `effective_mode` reports what any given image will really get.

The null survives that relabelling, for the structural reason set out below:
GAT and Anscombe differ by a multiplicative constant plus a small offset
inside the root, and ZNCC normalises the multiplicative part away -- so the
two sides are near-equivalent under the score being computed whichever arm
each one lands in, and the identical 14/10 split above is what that predicts.
What has NOT been measured is a genuinely symmetric GAT, which would need a
template large enough to fit its own model (>= 160 px square at these
settings) or a coarser block; issue #13 should be read as closed against the
hybrid, not against symmetric GAT.
...
like the one that sank the Fourier-Mellin route
(`scripts/spectral_pose.py`, measured negative), not a tuning failure.
...
def _block_stats(img: np.ndarray, block: int = DEFAULT_BLOCK) -> tuple[np.ndarray, np.ndarray]:
...
def estimate_noise_model(img: np.ndarray, block: int = DEFAULT_BLOCK,
...
def can_fit_noise_model(shape, block: int = DEFAULT_BLOCK,
                        bins: int = DEFAULT_BINS) -> bool:
    """Whether an image of this shape has enough blocks to fit (a, b) at all.

    A size test, not a goodness test: `estimate_noise_model` needs `4 * bins`
    block samples before it will attempt a fit, so an image below that floor
    returns (0, 0) for every possible set of pixel values. Separated out
    because the answer is a property of the geometry, and the geometry of a
    template here is fixed -- see `effective_mode`.
    """
    rows = int(shape[0]) // block
    cols = int(shape[1]) // block
    return rows * cols >= 4 * bins


def effective_mode(img: np.ndarray, mode: str) -> str:
    """The mode `apply` will really use on this image.

    `gat` degrades to `anscombe` whenever the noise model cannot be fitted, and
    that degradation is invisible at the call site. It is not a rare edge case
    here: a rendered template is TEMPLATE_SIZE = 100 px square, which is 6x6 =
    36 blocks at the default 16 px block against a floor of 4 * 24 = 96, so a
    template can *never* fit its own model regardless of its content. Any
    caller reporting which arm it measured has to ask this rather than assume
    the mode it passed in.
    """
    if mode != "gat":
        return mode
    if not can_fit_noise_model(img.shape):
        return "anscombe"
    a, _ = estimate_noise_model(img)
    return "gat" if a > 0.0 else "anscombe"
...
def apply_pair(reference: np.ndarray, search: np.ndarray,
    ...
    Both images must be transformed, and under the same nominal mode: ZNCC is
    invariant to an affine intensity relation between template and window, and
    a VST applied to only one side destroys that relation outright.

    `gat` is nominally per-image -- reference and search are separate
    acquisitions at different dose (`dose_reference` vs `dose_search` in the
    generator manifest), so they genuinely have different (a, b). Only the
    search side can actually be fitted, though: a rendered template is
    TEMPLATE_SIZE = 100 px square, 6x6 = 36 blocks at the default 16 px block
    against `estimate_noise_model`'s floor of 4 * 24 = 96, so the template
    falls back to plain Anscombe for every possible input. The `gat` arm is
    therefore **GAT(search) + Anscombe(template)** by construction rather than
    symmetric GAT, and the measured table in the module docstring is labelled
    accordingly. Call `effective_mode` rather than assuming the mode passed in.
    """
