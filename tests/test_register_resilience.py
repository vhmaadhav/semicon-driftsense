...
def test_per_pair_csv_flush_is_not_gated_on_the_display_flag():
    """Crash durability must not depend on a cosmetic flag.

    The per-pair `f.flush()` used to sit inside `if not a.quiet:`, so
    `--quiet` silently bought a weaker guarantee: rows stayed in the block
    buffer and a kill mid-run lost a whole block rather than the pair in
    flight. The graded command passes no flags, so this was latent rather
    than live -- but the coupling was accidental, and the fix is only worth
    anything if it cannot be re-nested by a later edit.

    Asserted structurally rather than behaviourally: provoking a real
    mid-run kill is inherently flaky, while "the flush is not inside the
    quiet branch" is exactly the property that regressed.
    """
    import ast

    with open(os.path.join(REPO_ROOT, "register.py"), encoding="utf-8") as fh:
        tree = ast.parse(fh.read())

    main = next(n for n in ast.walk(tree)
                if isinstance(n, ast.FunctionDef) and n.name == "main")

    def flushes_csv(node):
        """`f.flush()` calls anywhere beneath `node`."""
        found = []
        for sub in ast.walk(node):
            if (isinstance(sub, ast.Call)
                    and isinstance(sub.func, ast.Attribute)
                    and sub.func.attr == "flush"
                    and isinstance(sub.func.value, ast.Name)
                    and sub.func.value.id == "f"):
                found.append(sub)
        return found

    assert flushes_csv(main), (
        "register.py no longer flushes the predictions file per pair; the "
        "output contract's durability guarantee is unpinned")

    for node in ast.walk(main):
        if not isinstance(node, ast.If):
            continue
        mentions_quiet = any(
            isinstance(sub, ast.Attribute) and sub.attr == "quiet"
            for sub in ast.walk(node.test))
        if not mentions_quiet:
            continue
        nested = [c for stmt in node.body for c in flushes_csv(stmt)]
        assert not nested, (
            "f.flush() is nested inside a branch on --quiet at register.py "
            "line " + str(nested[0].lineno) + "; crash durability must not "
            "depend on a display flag")
